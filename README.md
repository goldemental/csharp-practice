# hello-world

This repository is used to practice GitHub Flow and improve my C# development skills.

## About Me

I am a university student currently learning C#.  
In this repository, I will share small daily projects as part of my 30-day C# practice plan.  
My goal is to build a strong understanding of fundamental concepts and become more confident in C#.

════════════════════════════════════════

**Day 1 – Hello World & Basic Input/Output**

A simple console application that:
- Prints "Hello World"
- Asks the user for their name
- Asks the user for their age
- Displays the entered information using string formatting

Concepts practiced:
- Console.WriteLine
- Console.ReadLine
- Variables
- int.Parse
- String formatting ({0}, {1})

[You can check out the code I wrote for Day 1 here](./Day01/HelloWorld/HelloWorld/Program.cs)

**Day 2 – Data Types & User Input**
A small console program that asks the user for two numbers and prints their sum.

**Day 3 – Operators**
Write a program that calculates the area of a rectangle using user input for length and width.

**Day 4 – Decision Structures (if-else)**
Create a program that checks if a number entered by the user is even or odd.

**Day 5 – Switch Statement**
Write a program that asks the user for a day number (1-7) and prints the corresponding day name.

**Day 6 – Loops: for**
Write a program that prints numbers from 1 to 10 using a for loop.

**Day 7 – Loops: while**
Write a program that asks the user for a number and prints all numbers from 1 to that number.

**Day 8 – Loops: foreach**
Create an array of names and print each name using a foreach loop.

**Day 9 – Arrays**
Write a program that stores 5 numbers in an array and prints their sum.

**Day 10 – Lists**
Create a list of your favorite fruits and print them.

**Day 11 – String Methods**
Write a program that asks for the user's full name and prints it in uppercase.

**Day 12 – String Concatenation**
Ask for the user's first and last name and print a greeting message using concatenation.

**Day 13 – Math Class**
Write a program that calculates the square root of a number entered by the user.

**Day 14 – Random Numbers**
Create a program that generates a random number between 1 and 100 and asks the user to guess it.

**Day 15 – Simple Calculator**
Ask the user for two numbers and an operator (+, -, *, /) and print the result.

**Day 16 – Functions/Methods**
Create a function that takes a number and returns its square. Test it in Main.

**Day 17 – Return Values**
Write a method that takes two numbers and returns the larger one.

**Day 18 – Parameters**
Create a method that greets a user by name.

**Day 19 – Boolean Methods**
Write a method that checks if a number is positive.

**Day 20 – Try/Catch Exception**
Ask the user for a number and handle invalid input using try/catch.

**Day 21 – Simple Calculator with Methods**
Refactor Day 15’s calculator to use methods for each operation.

**Day 22 – 2D Arrays**
Create a 2x2 matrix and print its elements.

**Day 23 – Loops with Arrays**
Ask the user for 5 numbers, store them in an array, and print the sum using a loop.

**Day 24 – Lists and Loops**
Create a list of 5 numbers and print all numbers greater than 3.

**Day 25 – String Formatting**
Ask for a name and age and print them using string interpolation.

**Day 26 – DateTime**
Write a program that prints today’s date and the day of the week.

**Day 27 – Simple Game**
Create a number guessing game with feedback if the guess is too high or too low.

**Day 28 – More Practice with Methods**
Write a method that returns the factorial of a number.

**Day 29 – Nested Loops**
Print a multiplication table from 1 to 5 using nested loops.

**Day 30 – Final Mini Project**
Combine what you learned: ask the user for a few numbers, calculate sum, average, and display results nicely.
